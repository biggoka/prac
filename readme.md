...
При открытии интерфеса пользователь получает приглашение ввести свой логин. Если пользователь не зарегистрирован в системе, то он может получить свой логин от администратора.

	$> Plese log in: 

При успешной авторизации пользователь получает список возможных комманд:

	$> Please enter one of the following commands:
	$>   list_rooms 
	$>   list_groups
	$>   list_requests
	$>   add_request [subject] [group-id] [room-id] [wd(>,<,>=,<=,==)(1,2,3,4,5,6)] [pn(>,<,>=,<=,==)(1-6)]
	$>   clear_requests

Описание комманд:
	 
- **list rooms** 
	Выводит список всех аудиторий.

- **list groups** 
	Выводит список всех академических групп.

- **list subjects**  
	Выводит список аудиторий.

- **list requests** 
	Выводит список запросов, которые пользователь уже ввел в систему.

- **add request**  
	Эта комманда позволяет пользователю добавить запрос. Она принимает следующие параметры:
	- [group-id] - id группы, для которой формируется запрос.
	- [subject-id] - id предмета, для которого формируется запрос.
	- [room-id] - id аудитории, для которой формируется запрос.
	- [wd:(ln,),] - этот параметр задает желаемое время для проведения занятия.
		*wd* - номер дня недели (1-7)
		*ln* - номер пары (1-6)
		Для каждого дня недели может быть перечислено несколько номеров пар через запятую.

		Например, `1:(1,3,4)` означает, что желаемое время - понедельник, первая, третья и четвертая пары.
